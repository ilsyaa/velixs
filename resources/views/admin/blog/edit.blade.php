    ...
    <div class="app-content content ">
        ...
        <div class="content-wrapper container-xxl p-0">
            ...
            <div class="content-body">
                <form class="submit-form" action="{{ route('blogs.update', $blog->id) }}" method="post" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="card">
                    ...
                    <div class="row">
                                    ...
                                    <div class="tab-content pt-1">
                                        ...
                                        <div class="tab-pane active" id="general-fill" role="tabpanel" aria-labelledby="general-tab-fill">
                                            ...
                                            <div class="mb-1">
                                                ...
                                                <input class="form-control" type="text" name="title" value="{{ old('title') ? old('title') : $blog->title }}" autofocus />
                                            </div>
                                            <div class="mb-1">
                                                <label class="form-label" for="title">Slug</label>
                                                <input class="form-control" type="text" name="slug" value="{{ old('slug') ? old('slug') : $blog->slug }}" />
                                            </div>
                                        ...
                                        <div class="tab-pane" id="meta-fill" role="tabpanel" aria-labelledby="meta-tab-fill">
                                            <div class="card border-1">
                                                <div class="card-body">
                                                    <h5 id="preview_meta_title" class="text-primary mb-0">{{ old('meta_title') ? old('meta_title') : $blog->meta_title }}</h5>
                                                    <small class="mb-1" style="color: #006621">{{ url('') }}/blog/<span id="preview_meta_slug">{{ old('slug') ? old('slug') : $blog->slug }}</span></small><br>
                                                    <small><span class="text-muted">{{ $blog->published_at ? date('M d, Y', strtotime($blog->published_at)) . ' - ' : '' }} </span><span id="preview_meta_description">{{ old('meta_description') ? old('meta_description') : $blog->meta_description }}</span></small>
                                                </div>
                                            </div>
                                            <hr>
                                            <h6>Edit SEO meta</h6>
                                            <div class="mb-1">
                                                ...
                                                <input class="form-control" type="text" name="meta_title" value="{{ old('meta_title') ? old('meta_title') : $blog->meta_title }}" />
                                            </div>
                                            <div class="mb-1">
                                                <label class="form-label" for="meta_title">Meta Description</label>
                                                <textarea class="form-control" name="meta_description" rows="3">{{ old('meta_description') ? old('meta_description') : $blog->meta_description }}</textarea>
                                            </div>
                                            <div class="mb-1">
                                                <label class="form-label" for="meta_title">Meta Keywords</label>
                                                <input class="form-control" type="text" name="meta_keywords" value="{{ old('meta_keywords') ? old('meta_keywords') : $blog->meta_keywords }}" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-12 col-xl-4 col-lg-4">
                                ...
                                <div class="card-body p-1">
                                    ...
                                    <select class="select2 form-select" name="category" id="select2-basic">
                                        @foreach ($categories as $cat)
                                            <option {{ (old('category') ? (old('category') == $cat->id ? 'selected' : '') : $blog->category_id == $cat->id) ? 'selected' : '' }} value="{{ $cat->id }}">{{ $cat->title }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="card">
                                <div class="card-body p-1">
                                    <img class="card-img mb-1" id="preview" src="{{ $blog->_thumbnail() }}" alt="">
                                ...
                                <div class="card-body p-1">
                                    <div class="mb-1">
                                        ...
                                        <select class="form-select" name="ctype">
                                            <option {{ (old('ctype') ? (old('ctype') == 'article' ? 'selected' : '') : $blog->content_type == 'article') ? 'selected' : '' }} value="article">Article</option>
                                            <option {{ (old('ctype') ? (old('ctype') == 'tutorial' ? 'selected' : '') : $blog->content_type == 'tutorial') ? 'selected' : '' }} value="tutorial">Tutorial</option>
                                        </select>
                                    </div>
                                    <div class="mb-1">
                                        <small class="">Status</small>
                                        <select class="form-select" name="status">
                                            <option {{ (old('status') ? (old('status') == 'publish' ? 'selected' : '') : $blog->published_at != null) ? 'selected' : '' }} value="publish">Published</option>
                                            <option {{ (old('status') ? (old('status') == 'draft' ? 'selected' : '') : $blog->published_at == null) ? 'selected' : '' }} value="draft">Draft</option>
                                        </select>
                                    </div>
                                    <div class="mb-1">
                                        ...
                                        <select class="select2 form-select" id="select2-nested" name="tags[]" multiple>
                                            @foreach ($tags as $tag)
                                                <option {{ (old('tags') ? (in_array($tag->id, old('tags')) ? 'selected' : '') : $blog->tags->contains($tag->id)) ? 'selected' : '' }} value="{{ $tag->id }}">{{ $tag->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
        ...
        tinymce.init({
            ...
            setup: function(editor) {
                editor.on('init', function(e) {
                    editor.setContent(`{!! old('body') ? old('body') : $blog->body !!}`);
                });
            },
        });
    </script>
