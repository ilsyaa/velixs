    ...
    <div class="app-content content ">
        ...
        <div class="content-wrapper container-xxl p-0">
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-start mb-0">Edit Page</h2>
            ...
            <div class="content-body">
                <form class="submit-form" action="{{ route('pages.update', $page->id) }}" method="post" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="card">
                                    ...
                                    <div class="tab-content pt-1">
                                        ...
                                        <div class="tab-pane active" id="general-fill" role="tabpanel" aria-labelledby="general-tab-fill">
                                            ...
                                            <div class="mb-1">
                                                ...
                                                <input class="form-control" type="text" name="title" value="{{ old('title') ? old('title') : $page->title }}" autofocus />
                                            </div>
                                            <div class="mb-1">
                                                <label class="form-label" for="title">Slug</label>
                                                <input class="form-control" type="text" name="slug" value="{{ old('slug') ? old('slug') : $page->slug }}" />
                                            </div>
                                        ...
                                        <div class="tab-pane" id="meta-fill" role="tabpanel" aria-labelledby="meta-tab-fill">
                                            <div class="card border-1">
                                                <div class="card-body">
                                                    <h5 id="preview_meta_title" class="text-primary mb-0">{{ old('meta_title') ? old('meta_title') : $page->meta_title }}</h5>
                                                    <small class="mb-1" style="color: #006621">{{ url('') }}/blog/<span id="preview_meta_slug">{{ old('slug') ? old('slug') : $page->slug }}</span></small><br>
                                                    <small><span class="text-muted">{{ $page->published_at ? date('M d, Y', strtotime($page->published_at)) . ' - ' : '' }} </span><span id="preview_meta_description">{{ old('meta_description') ? old('meta_description') : $page->meta_description }}</span></small>
                                                </div>
                                            </div>
                                            <hr>
                                            <h6>Edit SEO meta</h6>
                                            <div class="mb-1">
                                                ...
                                                <input class="form-control" type="text" name="meta_title" value="{{ old('meta_title') ? old('meta_title') : $page->meta_title }}" />
                                            </div>
                                            <div class="mb-1">
                                                <label class="form-label" for="meta_title">Meta Description</label>
                                                <textarea class="form-control" name="meta_description" rows="3">{{ old('meta_description') ? old('meta_description') : $page->meta_description }}</textarea>
                                            </div>
                                            <div class="mb-1">
                                                <label class="form-label" for="meta_title">Meta Keywords</label>
                                                <input class="form-control" type="text" name="meta_keywords" value="{{ old('meta_keywords') ? old('meta_keywords') : $page->meta_keywords }}" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-12 col-xl-4 col-lg-4">
                            <div class="card">
                                <div class="card-body p-1">
                                    <img class="card-img mb-1" id="preview" src="{{ $page->_thumbnail() }}" alt="">
                            ...
                            <div class="card">
                                <div class="card-body p-1">
                                    ...
                                    <select class="form-select" name="status">
                                        <option {{ (old('status') ? (old('status') == 'publish' ? 'selected' : '') : $page->published_at != null) ? 'selected' : '' }} value="publish">Published</option>
                                        <option {{ (old('status') ? (old('status') == 'draft' ? 'selected' : '') : $page->published_at == null) ? 'selected' : '' }} value="draft">Draft</option>
                                    </select>
                                </div>
                            </div>
                            <div class="card">
                                <div class="card-body p-1">
                                    <h5 class="card-title">Meta Robots</h5>
                                    <select class="form-select" name="robots">
                                        <option {{ (old('robots') ? (old('robots') == 'index' ? 'selected' : '') : $page->meta_robots == 'index') ? 'selected' : '' }} value="index">index, follow</option>
                                        <option {{ (old('robots') ? (old('robots') == 'noindex' ? 'selected' : '') : $page->meta_robots == 'noindex') ? 'selected' : '' }} value="noindex">noindex, nofollow</option>
                                    </select>
                                </div>
                            </div>
                        </div>
        ...
        tinymce.init({
            ...
            setup: function(editor) {
                editor.on('init', function(e) {
                    editor.setContent(`{!! old('body') ? old('body') : $page->body !!}`);
                });
            },
        });
    </script>
